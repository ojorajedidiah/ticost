...
function getClientRecords()
{
  ...
  try {
    ...
    if ($db->isLastQuerySuccessful()) {
      ...
      $sql = "SELECT clientID,clientName,clientNumber,clientWhatsAppNum,clientAddress FROM dkn_clients 
      ...
      foreach ($stmt->fetchAll() as $row) {
        ...
        $rtn .= '<tr><td>' . $row['clientName'] . '</td><td>' . $row['clientNumber'] . '</td>'
          . '<td>' . $row['clientWhatsAppNum'] . '</td><td>' . substr($row['clientAddress'],0,15) . '</td>'
